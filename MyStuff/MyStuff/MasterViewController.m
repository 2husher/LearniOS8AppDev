...
- (void)insertNewObject:(id)sender
{
    ...
    static NSUInteger itemNumber = 1;
    NSString *newItemName = [NSString stringWithFormat:@"My Item %u", itemNumber++];
    MyWhatsit *newItem = [[MyWhatsit alloc] initWithName:newItemName
                                                location:nil];
    [self.things insertObject:newItem
                      atIndex:0];
...
- (void)whatsitDidChangeNotification:(NSNotification*)notification
{
    ...
    if (index != NSNotFound)
    {
        NSIndexPath *path = [NSIndexPath indexPathForItem:index
                                                inSection:0];
